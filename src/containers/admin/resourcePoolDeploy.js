...
class ResourcePoolDeploy extends Component {
    constructor(props) {
        super(props);
        this.state={
            page:1,        //列表页码
    ...
    getList=()=>{
        ...
        this.setState({dataLoading:true},()=>{
            request({
                url:'/SysConfig/listResourceConfig',
                method:'post',
    ...
    render() {
        const columns = [ {title: '序号',dataIndex: 'key'},{title: '资源池',dataIndex: 'grade',render:(text)=>(<span>{this.grade(text)}</span>)},
            {title: '所属公司',dataIndex: 'company_name'},{title: '容量',dataIndex: 'capacity'},{title: '保护期',dataIndex: 'protection_period'},
            {title: '操作',key: 'action',
...
export default ResourcePoolDeploy;